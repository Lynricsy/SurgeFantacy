...
// 服务商后缀识别正则表达式 🎯 - 支持有数字和无数字两种格式
const providerSuffixWithNumber = /(\d+)(\s+.+)$/;  // 格式: HK1 Eons
const providerSuffixWithoutNumber = /^([A-Za-z]+)(\s+.+)$/;  // 格式: HK Eons
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 🎯 提取服务商后缀 - 支持有数字和无数字两种格式
    let providerSuffix = "";

    // 先尝试匹配有数字的格式: HK1 Eons
    let suffixMatch = e.name.match(providerSuffixWithNumber);
    if (suffixMatch) {
      const digitPart = suffixMatch[1]; // 数字部分 (如 1, 13)
      const suffixPart = suffixMatch[2]; // 后缀部分 (如 " Eons")

      providerSuffix = suffixPart; // 保存完整后缀（包含前导空格）
      // 从原节点名中移除后缀，保留到数字结束
      e.name = e.name.replace(providerSuffixWithNumber, digitPart);
      e.providerSuffix = providerSuffix.trim(); // 存储后缀（用于排序识别）
    } else {
      // 再尝试匹配无数字的格式: HK Eons
      suffixMatch = e.name.match(providerSuffixWithoutNumber);
      if (suffixMatch) {
        const regionPart = suffixMatch[1]; // 地区部分 (如 HK, SG)
        const suffixPart = suffixMatch[2]; // 后缀部分 (如 " Eons")
