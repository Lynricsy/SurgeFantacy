...
// 服务商后缀识别正则表达式 🎯 - 识别第一个数字后面的所有内容
const providerSuffixRegex = /(\d+)(.+)/;
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 🎯 提取服务商后缀 - 第一个数字后面的所有内容
    let providerSuffix = "";
    const suffixMatch = e.name.match(providerSuffixRegex);
    if (suffixMatch) {
      const digitPart = suffixMatch[1]; // 数字部分
      const suffixPart = suffixMatch[2]; // 数字后面的内容
      providerSuffix = suffixPart; // 保存后缀
      // 从原名称中移除后缀，保留到数字结束
      const digitEndIndex = e.name.indexOf(digitPart) + digitPart.length;
      e.name = e.name.substring(0, digitEndIndex);
