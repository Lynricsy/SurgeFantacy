 ...
 * 新增功能: 服务商后缀自动识别与保留 (第一个数字后的所有内容都会被保留并优先排序)
 */
...
// 服务商后缀识别正则表达式 🎯 - 识别数字后的服务商后缀（更灵活的匹配）
const providerSuffixRegex = /(\d+)(\s+.+)$/;
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 🎯 提取服务商后缀 - 识别数字后的空格+后缀格式
    let providerSuffix = "";
    const suffixMatch = e.name.match(providerSuffixRegex);
    if (suffixMatch) {
      ...
      // 从原节点名中移除后缀，保留到数字结束
      e.name = e.name.replace(providerSuffixRegex, digitPart);
      e.providerSuffix = providerSuffix.trim(); // 存储后缀（用于排序识别，去掉前导空格）
    }
