...
// 服务商后缀识别 🎯 - 使用字符串分割方法，支持 HK1 Eons 和 HK Eons 两种格式
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    if (e.name.includes(' ')) {
      ...
      if (parts.length >= 2) {
        const firstPart = parts[0]; // 第一部分 (如 HK, HK1, SG13)
        const restParts = parts.slice(1).join(' '); // 后面的部分 (如 HKT, Eons)

        // 检查第一部分的格式
        const numberMatch = firstPart.match(/^([A-Za-z]+)(\d+)$/); // 格式: HK1, SG13
        if (numberMatch) {
          // 有数字的格式: HK1 Eons
          const regionCode = numberMatch[1]; // HK
          const number = numberMatch[2]; // 1
          e.name = regionCode + number; // HK1
          e.providerSuffix = restParts; // Eons
        } else if (/^[A-Za-z]{2,3}$/.test(firstPart)) {
          // 无数字的格式: HK Eons (2-3个字母)
          e.name = firstPart; // HK
          e.providerSuffix = restParts; // HKT
        }
      }
    }
